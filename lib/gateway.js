...
function adjustRequestData(req) {
    ...
    if(req.data.params.group) {
        ...
        req.data.group = req.data.params.group;

        if(req.data.group.condition) {

            parserUtil.parseReservedKey(req.data.group.condition);
        }
    }

    if(req.data.params.aggregate) {

        req.data.aggregate = req.data.params.aggregate;
